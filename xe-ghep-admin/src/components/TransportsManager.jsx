...
const TransportsManager = () => {
    ...
    const sortedTransports = [...transports].sort((a, b) => {
      ...
      switch (column) {
        ...
        case 'dropoffLocation':
          aValue = a.dropoffLocation.toLowerCase();
          bValue = b.dropoffLocation.toLowerCase();
          break;
        case 'date':
          aValue = new Date(a.transportDate);
          bValue = new Date(b.transportDate);
          break;
        case 'phoneNumber':
  ...
  const saveModal = async () => {
    try {
      ...
      const url = currentTransport
        ...
        : `${process.env.REACT_APP_API_URL}/transport/RegisterTransport`;
      ...
      if (currentTransport) {
        const updatedTransports = transports.map((t) =>
          t._id === currentTransport._id
            ? { ...currentTransport, status: modalStatus, note: modalNote }
            : t
        );
  ...
  const convertStatus = (status) => {
    switch (status) {
      case '0': return '📦Chưa lấy hàng';
      case '1': return '🚚Đã lấy hàng';
      case '2': return '✅Đã trả hàng';
      case '3': return '🚫Đã hủy';
      case '4': return '⏩Đã chuyển nhượng';
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold">
              Transport List{" "}
              <span className="text-sm font-normal text-gray-600">({transports.length} items)</span>
            </h2>
            <button
              onClick={() => openModal()}
            ...
            <table className="min-w-full divide-y divide-gray-200 text-xs sm:text-sm table-fixed w-[1150px]">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="w-[150px] px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <button onClick={() => handleSort('dropoffLocation')} className="text-indigo-600 hover:text-indigo-800 uppercase">
                      Điểm trả <span>{sortConfig.column === 'dropoffLocation' ? (sortConfig.ascending ? '↑' : '↓') : ''}</span>
                    </button>
                  </th>
              ...
              <tbody className="divide-y divide-gray-200">
                {transports.map((transport) => (
                  <tr
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && openModal(transport)}
                  >
                    ...
                    <td className="px-2 py-2 whitespace-nowrap truncate">
                      <a href={`tel:${transport.phoneNumber}`} className="text-blue-600 hover:underline" onClick={(e) => e.stopPropagation()}>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-40">
          <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-md">
            ...
            <table className="mb-4 min-w-full text-sm text-left text-gray-700 table-fixed">
              <tbody>
                {currentTransport && (
                  <>
                    <tr>
                      <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Transport ID</th>
                      <td className="py-1 break-words whitespace-normal">{currentTransport._id}</td>
                    </tr>
                    <tr>
                      <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Loại</th>
                ...
                {!currentTransport && (
                  <tr>
                    ...
                    <td className="py-1 break-words whitespace-normal">
                      <input
                        type="text"
                        placeholder="Loại xe (5cho, 7cho, xetai...)"
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        onChange={(e) => setCurrentTransport({ ...(currentTransport || {}), type: e.target.value })}
                      />
                    </td>
                  </tr>
                )}
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Điểm đón</th>
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Điểm trả</th>
                  <td className="py-1 break-all whitespace-normal">
                    <input
                      type="text"
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Ngày vận chuyển</th>
                  <td className="py-1 break-words whitespace-normal">
                    <input
                      type="datetime-local"
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Số tiền</th>
                  <td className="py-1 break-words whitespace-normal">
                    <div className="relative">
                      <input
                        type="text"
                        value={currentTransport?.amount ? currentTransport.amount.toLocaleString('vi-VN') : ''}
                        onChange={(e) => {
                          const raw = e.target.value.replace(/\D/g, '');
                          setCurrentTransport({ ...currentTransport, amount: raw ? parseInt(raw, 10) : 0 });
                        }}
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Số điện thoại</th>
                  <td className="py-1 break-words whitespace-normal">
                    <input
                      type="tel"
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Số người</th>
                  <td className="py-1 break-words whitespace-normal">
                    <input
                      type="number"
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Ghi chú</th>
                  <td className="py-1 break-all whitespace-normal">
                    <textarea
                      value={modalNote}
                ...
                <tr>
                  <th className="w-40 pr-4 py-1 font-medium text-gray-700 align-top">Trạng thái</th>
                  <td className="py-1 break-words whitespace-normal">
                    <select
                      value={modalStatus}
