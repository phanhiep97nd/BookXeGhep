...
const FormRegister = ({ UpdateTypeofCar, TypeChecked, CheckHoliday , UpdateDate}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.date) {
            ...
            const now = new Date();
            if (selectedDate < now) {
                alert('Ngày giờ đón không thể là thời điểm trong quá khứ!');
                return;
            }
        }
        if (!formData.pickupFrom) {
            alert('Vui lòng nhập điểm đón!');
            return;
        }
        if (!formData.destination) {
            alert('Vui lòng nhập điểm trả!');
            return;
        }
        setIsModalOpen(true);
    };
    ...
    const handleConfirm = async () => {
        const data = {
            ...formData,
            numberOfGuest: parseInt(formData.numberOfGuest, 10)
        };
        setIsLoading(true);
        try {
			 ...
			 <>
				...
				<form action="" className="hero-form" onSubmit={handleSubmit}>
					...
					<div className="input-wrapper">
						...
						<input type="text" name="pickupFrom" id="PickupFrom" className="input-field" placeholder="Nhập điểm đón tại đây..." value={formData.pickupFrom} onChange={handleChange}/>
						<label	className="input-label">----------------------------</label>
					...
					<div className="input-wrapper">
						...
						<input type="tel" name="phoneNumber" id="inputPhoneNumber" className="input-field" placeholder="Nhập SDT tại đây (VD: 0901234567)..." value={formData.phoneNumber} onChange={handleChange} maxLength={11}/>
						<label	className="input-label">----------------</label>
